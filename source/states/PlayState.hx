...
class PlayState extends MusicBeatState
{
	...
	{
		@:privateAccess
		if (FlxG.sound.music == null || FlxG.sound.music._channel == null || FlxG.sound.music._channel.__audioSource == null)
			return;
	...
	override public function create()
	{
		...
		Paths.clearStoredMemory();
 		if (SONG != null && initialMania == -114)
        	initialMania = SONG.mania;
		// 移动 fpsVar 向右 150px
		// 暂时不用了
		...
		timeBar = new Bar(0, timeTxt.y + (timeTxt.height / 4), ClientPrefs.data.timebarStyle == "Kade" ? 'timeBarKE' : 'timeBar',
			function() return songPercent, 0, 1);
		...
		{
			// 其他样式保持默认
			timeBar.setColors(0xFFFFFF, 0x000000); // 黑色和白色
		...
		curIconP1 = boyfriend.healthIcon;
		...
		botplayTxt = new FlxText(400, ClientPrefs.data.botplayStyle == 'Kade' ? healthBar.y - 120 : timeBar.y + 55, FlxG.width - 800,
			ClientPrefs.data.botplayStyle == 'Kade' ? "BOTPLAY" : 'AUTOPLAY', 32);
		botplayTxt.setFormat(Paths.font("vcr.ttf"), ClientPrefs.data.botplayStyle == 'Kade' ? 37 : 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE,
			FlxColor.BLACK);
		...
		// 添加水印文本
		var watermarkText = new FlxText(20, FlxG.height
			- 20, 0,
			SONG.song
			+ "-"
	...
	public function reloadHealthBarColors()
{
	...
	public function updateScoreText(miss:Bool = false)
	{
		...
		{
			tempScore = 'NPS: ${nps} (${maxNPS}) | Score: ${songScore}' + (!instakillOnMiss ? ' | Miss: ${songMisses}' : "")
				+ ' | Acc: ${percent}% | ${ratingFC}' + (cpuControlled ? ' | AUTOPLAY' : "");
		}
	...
	{
		var returnedValue:Null<Float> = callOnScripts('eventEarlyTrigger', [
			event.event,
			event.value1,
			event.value2,
			event.value3,
	...
	public dynamic function updateIconsPosition()
	{
...

		{
			...
			if (ClientPrefs.data.iconbopstyle == "VSlice(Old)")
			{
	...
	{
		if (healthBar == null || healthBar.valueFunction == null)
			return;
	...
	function openChartEditor()
	{
		...
		// 进入制谱器还原初始的mania值，以防止出现问题
		if (SONG != null && initialMania != -114)
        SONG.mania = initialMania;
    ...
    switch (charType)
    {
        case 0:
			...
			if (ClientPrefs.data.iconbopstyle == "MintRhythm")	iconP1.angle += 270; // 重置角度
            curIconP1 = value2; // 记录当前icon名
        case 1:
            iconP2.changeIcon(value2);
            if (ClientPrefs.data.iconbopstyle == "MintRhythm")	iconP2.angle -= 270; // 重置角度
            curIconP2 = value2;
        case 2:
            if (gf != null)
                gf.healthIcon = value2;
    }
    reloadHealthBarColors();
	...
	private function cachePopUpScore()
	{
		...
		for (rating in ratingsData)
			Paths.image(uiPrefix + rating.image + uiSuffix + ratingexspr);
		for (theEXrating in ratingsData)
			Paths.image(uiPrefix + theEXrating.image + uiSuffix + exratingexspr);
		for (i in 0...10)
			Paths.image(uiPrefix + 'num' + i + uiSuffix + numexspr);
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!ClientPrefs.data.rmmsTimeTxt)
		{
			...
			// 调整显示格式，保留两位小数
			if (cpuControlled)
				msTimeTxt.text = Std.string(CoolUtil.floorDecimal(noteDiff, 2)) + "ms(BOT)";
			else
				msTimeTxt.text = Std.string(CoolUtil.floorDecimal(noteDiff, 2)) + "ms";

			if (msTimeTxtTween1 != null)
			{
				msTimeTxtTween1.cancel();
				msTimeTxtTween1.destroy(); // top 10 awesome code
			}
			msTimeTxtTween1 = FlxTween.tween(msTimeTxt, {alpha: 0}, (60 / Conductor.bpm) * 1.5, {
				onComplete: function(tw:FlxTween)
				{
					msTimeTxtTween1 = null;
				},
				startDelay: (60 / Conductor.bpm) * 0.7
			});

			if (msTimeTxtTween2 != null)
			{
				msTimeTxtTween2.cancel();
				msTimeTxtTween2.destroy(); // top 10 awesome code
			}
		...
		if (ClientPrefs.data.exratbounce == true && ClientPrefs.data.exratingDisplay)
		{
			...
			FlxTween.tween(theEXrating, {angle: targetAngle}, 0.025, {
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if (note.hitCausesMiss)
		{
			...
			noteMiss(note);
			if (!note.noteSplashData.disabled && !note.isSustainNote && ClientPrefs.data.cpuStrums)
				spawnNoteSplashOnNote(note);
			if (!note.isSustainNote)
				invalidateNote(note);
			return;
